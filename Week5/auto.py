...
min_lam = -1
min_k = -1
lamda1 = np.arange(0.01, 0.11, 0.01)
lamda2 = np.arange(20, 220, 20)
for i in range(2):
    for k in range(3):
        lamda = lamda1 if k < 2 else lamda2
        for lam in lamda:
            score = hw5.xval_learning_alg(hw5.make_polynomial_feature_fun(k+1)(auto_data[i]), auto_values, lam, 10) * sigma
            if score < min_rmse:
                min_rmse = score
                min_i = i + 1
                min_lam = lam
                min_k = k+1
            print("feature set", i+1, "order=", k+1, "lam:", lam, score)

print("min_rmse", min_rmse)   
print("min_i", min_i)
print("min_k", min_k) 
print("min_lam", min_lam)
