...
class Tanh(Module):  # Layer activation
    ...
    def backward(self, dLdA):  # Uses stored self.A
        # print("dlda", dLdA)
        # print("a", self.A)
        return dLdA * (1 - self.A ** 2)   # Your code: return dLdZ (?, b) -> (m, b)


class ReLU(Module):  # Layer activation
    def forward(self, Z):
        self.A =  Z * (Z > 0)# Your code: (?, b)
        return self.A

    def backward(self, dLdA):  # uses stored self.A
        return dLdA * (1.0 * (self.A > 0))  # Your code: return dLdZ (?, b)
    ...
    def mini_gd(self, X, Y, iters, lrate, notif_each=None, K=10):
        ...
        while num_updates < iters:
            ...
            for j in range(m.floor(N/K)):
                print("Iteration: ", num_updates)
                if num_updates >= iters: break
                print("Mini batch: ", j*K, ":", (j+1)*K)
...
def mini_gd_test():
    ...
    nn.mini_gd(X,Y, iters = 5, lrate=0.005, K=2)
