...
def perceptron(data, labels, params={}, hook=None):
    ...
    # data is d x n array
    # labels is 1 x n array
    d, n = data.shape   # dimenions of data
    ...
    for t in range(T):
        for i in range(n):
            y_i = labels[0,i]  # taking ith label, should be scaler
            x_i = data[:,i]    # taking ith data column, should be d x 1 column vector
...
for datafn in (super_simple_separable_through_origin,super_simple_separable):
   data, labels = datafn()
   test_linear_classifier(datafn,perceptron,draw=True, pause=True)
'''
...
def averaged_perceptron(data, labels, params={}, hook=None):
    ...
    # labels = labels.reshape(n, 1)  # change labels to n x 1
    # data = data.T                   # change data to n x d
    # print("data:\n", data)
    # print("labels:\n", labels)
...
for datafn in (super_simple_separable, xor, xor_more, big_higher_dim_separable):
   data, labels = datafn()
   test_linear_classifier(datafn,averaged_perceptron,draw=True, pause=True)
'''
...
def eval_learning_alg2(learner, data_gen, n_train, n_test, it):
    total = 0
    total_correct = 0
    for n in range(it):
        data_train, labels_train = data_gen(n_train)
        data_test, labels_test = data_gen(n_test)
        th, th0 = learner(data_train, labels_train)
        total_correct += score(data_train, labels_train, th, th0)
        total += data_train.shape[1]
    return float(total_correct / total)
...
def xval_learning_alg(learner, data, labels, k):
    ...
    for j in range(k):
        d_j = split_data[j]
        l_j = split_labels[j]
        d_minus_j = np.concatenate(split_data[:j] + split_data[j+1:], axis = 1)
        l_minus_j = np.concatenate(split_labels[:j] + split_labels[j+1:], axis = 1)
        th, th0 = learner(d_minus_j, l_minus_j)
        total_correct += score(d_j, l_j, th, th0)
        # total += eval_classifier(learner, d_minus_j, l_minus_j, d_j, l_j)
        total += d_j.shape[1]
        # print(d_minus_j)
...
print(eval_learning_alg2(perceptron, gen_flipped_lin_separable(pflip=.1), 20, 20, 100))

print(eval_learning_alg2(averaged_perceptron, gen_flipped_lin_separable(pflip=.1), 20, 20, 100))

print(eval_learning_alg2(perceptron, gen_flipped_lin_separable(pflip=.25), 20, 20, 100))

print(eval_learning_alg2(averaged_perceptron, gen_flipped_lin_separable(pflip=.25), 20, 20, 100))
